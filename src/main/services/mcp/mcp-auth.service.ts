...
export class MCPAuthService extends EventEmitter {
  ...
  async authorize(config: MCPOAuthConfig): Promise<OAuthTokens> {
    ...
    const redirectUri = config.redirectUri || 'chess-lens://oauth/callback';
  ...
  createAuthProvider(serverId: string, config: MCPOAuthConfig): OAuthClientProvider {
    ...
    return {
      get redirectUrl() {
        return config.redirectUri || 'chess-lens://oauth/callback';
      },
      ...
      get clientMetadata() {
        return {
          ...
          redirect_uris: [config.redirectUri || 'chess-lens://oauth/callback'],
        };
      },
